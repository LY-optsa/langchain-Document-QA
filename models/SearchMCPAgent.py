...
class MCPSearchTool:
    ...
    @staticmethod
    def _perform_search(query: str, max_results: int = 30) -> str:
        """
        ...
        try:
            ...
            results = asyncio.run(MCPSearchTool.searcher.search(query, ctx, max_results))
            return MCPSearchTool.searcher.format_results_for_llm(results)
    ...
    async def asearch(query: str = ""):
        ...
        try:
            ...
            ctx = MockContext()
            # 直接运行异步搜索
            results = await MCPSearchTool.searcher.search(query, ctx, 20)
            return MCPSearchTool.searcher.format_results_for_llm(results)
        except Exception as e:
